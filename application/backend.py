...
def lista_contas():
    ...
    # aplicar o método json que a classe Usuario possui a cada elemento da lista
    Usuario_em_json = [ x.json() for x in Usi ]
...
def filtroTipo_receita():
    ...
    # aplicar o método json que a classe Receitas possui a cada elemento da lista
...
def deletar_despesas(deldespesas):
    ...
    try:
        if delD.idusuario_despesas==valor:
            ...
            resposta = jsonify({"resultado": "erro", "detalhes": "Essa despesa não é sua"})
    except Exception as e:
        # informar mensagem de erro
        resposta = jsonify({"resultado":"erro", "detalhes":str(e)})
        # adicionar cabeçalho de liberação de origem
