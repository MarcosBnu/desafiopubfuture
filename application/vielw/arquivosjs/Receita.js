$(function () { // quando o documento estiver pronto/carregado
    ...
    $(document).on("click", "#btcadreceita", function () {
        ...
        $.ajax({
            ...
            success: cadastrar_receita, // chama a função cadastrar_receita para processar o resultado
            error: erroAoIncluir
        });
    ...
    $(document).on("click", "#btDelreceita", function () {
        ...
        function delreceita1 (retorno_cad) {
            if (retorno_cad.resultado == "ok") { // a operação deu certo?
                // informar resultado de sucesso
                alert("receita deletado com sucesso!");
                //recarrega a pagina
                location.reload();
                // limpar os campos
            } else {
    ...
    $(document).on("click", "#btFiltrodata", function () {
        ...
        function listar (fd1) {
            for (var i in fd1) { //i vale a posição no vetor
                ...
                if (FDtipo=="receber"){//verifica o que voce deseja filtrar
                    var date1 = new Date(fd1[i].Recebimento);
                    var Datainicial2 = new Date(Datainicial);
                    var Datafinal2 = new Date(Datafinal);
                    //tranforma as variaveis em datas
                    if (date1>Datainicial2 && date1<Datafinal2){//filtro de data
                        lin='<div style="margin: 30px; text-align: justify">'+
                ...
                if (FDtipo=="esperado"){//verifica o que voce deseja filtrar
                    var date2 = new Date(fd1[i].Esperado);
                    var Datainicial1 = new Date(Datainicial);
                    var Datafinal1 = new Date(Datafinal);
                    //tranforma as variaveis em datas
                    if (date2>Datainicial1 && date2<Datafinal1){//filtro de data
                        lin='<div style="margin: 30px; text-align: justify">'+
                            '<h4 class="card-title"> id da Receita: ' + fd1[i].idreceita +'</h4>'+
                            '<p class="card-text">Valor da Receita: R$ '+ fd1[i].Receita + '</p>'+
                            '<p class="card-text">Data de Recebimento: : '+ fd1[i].Recebimento + '</p>'+
...
$(function() { // quando o documento estiver pronto/carregado
    $(document).on("click", "#btLimp", function () {
        $.ajax({
            ...
            method: 'GET',
            dataType: 'json', // os dados são recebidos no formato json
            success: limpar_r, // chama a função limpar_r para processar o resultado
            error: function() {
                alert("erro ao ler dados, verifique o backend");
            }
        });
...
$(function() { // quando o documento estiver pronto/carregado
    $(document).on("click", "#btFiltrotipo", function () {
        ...
        $.ajax({
            ...
            dataType: 'json', // os dados são recebidos no formato json
            success: listar, // chama a função listar para processar o resultado
            error: function() {//se acontecer algo de errado, essa função é ativade
                alert("erro ao ler dados, verifique o backend");//mostra uma mensagem de erro
            }
        });
        ...
        function listar (fq) {
            for (var i in fq) { //i vale a posição no vetor
                //lin='<div class="card cores10card col-3">'+
                if (Ftipo=="Salário"){//verifica o tipo de receita que voce deseja filtrar
                    if (fq[i].Tipo_de_Receita=="Salário"){
                        lin='<div style="margin: 30px; text-align: justify">'+
                            ...
                            '</div>'+
                            '<br>';
                        $('#listar_filtrotipo').append(lin);//mostra na tela a variavel lin
                    }
                }
                if (Ftipo=="Presente"){//verifica o tipo de receita que voce deseja filtrar
                ...
                if (Ftipo=="Premio"){//verifica o tipo de receita que voce deseja filtrar
                ...
                if (Ftipo=="Outros"){//verifica o tipo de receita que voce deseja filtrar
