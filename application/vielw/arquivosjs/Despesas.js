...
$(function () { // quando o documento estiver pronto/carregado
    ...
    $(document).on("click", "#btDeldespesas", function () {
        ...
        $.ajax({
            ...
            type: 'DELETE',
            dataType: 'json', // os dados são recebidos no formato json
            success: deldespesas1, // chama a função deldespesas1 para processar o resultado
            error: erroAoIncluir
        });
                ...
                if (DFDtipo=="DPagamento"){
                    ...
                    if (date10>Datainicial2C && date10<Datafinal2C){
                        linhaFD='<div style="margin: 30px; text-align: justify">'+
                            ...
                            '</div>'+
                            '<br>';
                        $('#listar_Dfiltrodata').append(linhaFD);// adiciona a linha no corpo da tabela
                    }
                }
...
$(function() { // quando o documento estiver pronto/carregado
    ...
    function listar_totalDes (qw1) {
            linLTD='<div style="margin-left: 40%; text-align: left; margin-right: 35%;">'+
                '<h3 class="card-title"> Voce possui um total de ' + qw1.detalhes +' despesas cadastradas</h3>'+
                '</div>'+
                '<br>';
            // adiciona a linha no corpo da tabela
